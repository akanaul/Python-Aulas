# -*- coding: utf-8 -*-
"""
Suíte de Testes Unitários Automatizados — Issue #devtools
...
class TestIssue_DEVTOOLS(unittest.TestCase):
    """Testes automatizados comportamentais da Issue #devtools."""
        ...
        if not os.path.exists(self.manual_path):
            ...
            for root, dirs, files in os.walk(self.root_dir):
                if "devtools_mcp_manual.py" in files:
                    ...
                    found = True
                    break
            self.assertTrue(found or os.path.exists(self.manual_path), f"Arquivo devtools_mcp_manual.py da Issue #devtools não encontrado.")
        else:
            self.assertTrue(os.path.exists(self.manual_path))

    def test_file_syntax(self):
        """Valida a sintaxe Python do arquivo de prática da Issue #devtools."""
        if self.manual_path.endswith(".py") and os.path.exists(self.manual_path):
            ...
            try:
                ...
                self.fail(f"Erro de sintaxe Python na Issue #devtools: {e}")
