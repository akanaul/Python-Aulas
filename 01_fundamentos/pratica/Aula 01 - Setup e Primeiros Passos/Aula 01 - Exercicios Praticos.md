---
aliases: ["🧪 Exercícios Práticos — Aula 01: Setup Completo e Primeiros Passos"]
tags: [exercicio, pratica, issue-01, python]
---

# 🧪 Exercícios Práticos — Aula 01: Setup Completo e Primeiros Passos

### 📌 Do que se trata este Exercício?
Neste exercício, você validará a configuração do seu ambiente de desenvolvimento Python 3.12, ambiente virtual (`venv`) e saída de dados.

**Objetivo Prático:**
1. Abra o arquivo `aula_01_exercicios_manual.py` na sua IDE.
2. Escreva uma função que formate uma mensagem de boas-vindas ao sistema de logística usando **f-strings**, exibindo a versão ativa do Python e a data/hora atual de execução.
3. Garanta o uso correto de caracteres de escape e tratamento defensivo contra erros de `NameError` ou `SyntaxError`.

Clique no botão **Run** abaixo para validar seu setup no Obsidian!

---

## ⚡ Avaliação 1-Clique dos Exercícios da IDE

> [!EXERCICIO] 🧪 Avaliação 1-Clique dos Exercícios da IDE (Issue #01)
> 📌 **Exercício Avaliado:** Issue #01
> 📁 **Arquivo de Trabalho na IDE:** `01_fundamentos/pratica/Aula 01 - Setup e Primeiros Passos/aula_01_exercicios_manual.py`
> ⚡ Clique no botão **Run** no canto superior direito do bloco abaixo para testar sua solução:

```python run
import sys, os, subprocess

def find_vault_root():
    curr = os.path.abspath(os.getcwd())
    while curr:
        if os.path.exists(os.path.join(curr, "avaliar_exercicio.py")):
            return curr
        parent = os.path.dirname(curr)
        if parent == curr:
            break
        curr = parent
    user_home = os.path.expanduser("~")
    for root, dirs, files in os.walk(user_home):
        if "avaliar_exercicio.py" in files:
            return root
        if root.count(os.sep) - user_home.count(os.sep) >= 4:
            dirs.clear()
    return os.path.abspath(".")

vault_root = find_vault_root()
script_path = os.path.join(vault_root, "avaliar_exercicio.py")
print("📌 [AVALIAÇÃO 1-CLIQUE] Testando Exercício da Issue #01...")
print("📁 Arquivo Alvo na IDE: 01_fundamentos/pratica/Aula 01 - Setup e Primeiros Passos/aula_01_exercicios_manual.py")
res = subprocess.run([sys.executable, script_path, "--issue", "01"], cwd=vault_root, capture_output=True, text=True, encoding="utf-8", errors="replace")
print(res.stdout or res.stderr)
```

---

## 🔀 Workflow de Envio do Pull Request (PR)

Após passar em 100% nos testes automatizados acima:

```bash
# 1. Adicionar alterações ao staging
git add 01_fundamentos/pratica/Aula 01 - Setup e Primeiros Passos/aula_01_exercicios_manual.py

# 2. Registrar o commit
git commit -m "fix(issue-01): resolucao dos exercicios praticos"

# 3. Enviar para o repositório remoto
git push origin feature/issue-01
```

> 🚀 **Passo Final:** Abra o **Pull Request (PR)** no GitHub para revisão do Tutor!
